...
function formatBadge(key) {
  ...
  const mods = parts
    .slice(0, -1)
    .map((p) => ({ ctrl: "^", shift: "⇧", meta: "◆" })[p] ?? p.toUpperCase());
...
function buildLine(bindings, theme, padding) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const hint =
      items.length - i - 1 > 0 ? ` …+${items.length - i - 1}`.length : 0;
    if (used + extra + items[i].w + hint <= W) {
      used += extra + items[i].w;
      vis.push(i);
    } else break;
  }
  ...
  vis.forEach((idx, i) => {
    ...
    line +=
      fgKey + items[idx].badge + rst + " " + fgLbl + items[idx].label + rst;
  });
  if (hidden > 0)
    line += pad + fgSep + "│" + rst + pad + fgLbl + `…+${hidden}` + rst;
...
export function createHotkeyMenu(bindings, options = {}) {
  ...
  return {
    render,
    update(nb) {
      list = nb.map((b) => ({ ...b, _key: normalizeKey(b.key) }));
      render();
    },
    destroy,
  };
